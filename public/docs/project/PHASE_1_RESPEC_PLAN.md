...
1. Which fields are public coarse coordination metadata versus private exact matching data? Decision: public metadata is
   ...
   matching data. Principal and duration buckets are ceiling buckets, encoded as labels such as `P<000150` and
   `D<000720`; ranges are routed by the smallest ceiling that contains the range max, while exact overlap remains an
   encrypted matching check.
...
11. How should candidate discovery work? Decision: maintain an on-chain public bucket index of active/executable
    ...
    product feature, not a privacy failure. Bucket semantics are deliberately coarse `less-than-ceiling` routes rather
    than public min-max ranges; for example `[50, 100]` and `[50, 150]` principal ranges both route to `P<000150`.
